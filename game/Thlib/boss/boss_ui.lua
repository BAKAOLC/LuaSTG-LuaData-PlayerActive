...
function boss_ui:frame()--自机活版本
    task.Do(self)
    if self.countdown then
        if self.countdown>4 and self.countdown<=9 and self.countdown%1==0 then
            PlaySound('timeout',0.6)
        end
        if self.countdown>0 and self.countdown<=4 and self.countdown%1==0 then
            PlaySound('timeout2',1.0)
        end
    end
end

function boss_ui:oldrender()
    SetViewMode'world'
...
function boss_ui:render()--自机活版本
	...
	if self.is_combat then
		if self.hpbarlen or self.boss.ex then
			...
			if self.drawname then
				local w=lstg.world
				RenderTTF('boss_name',self.name,w.l+7,w.l+7,w.t-2-dy1,w.t-2-dy1,Color(255,0,0,0),'noclip')
				RenderTTF('boss_name',self.name,w.l+6,w.l+6,w.t-1-dy1,w.t-1-dy1,Color(255,255,128,255),'noclip')
				local m = int((self.sc_left-1)/8)
				if m >= 0 then
					for i=0,m-1 do
						for j=1,8 do
							Render('boss_ui_sc_left',w.l-2+j*12,w.t-17-i*12-dy1,0,0.2)
						end
					end
					for i=1,int(self.sc_left-1-8*m) do
						Render('boss_ui_sc_left',w.l-2+i*12,w.t-17-m*12-dy1,0,0.2)
					end
				end
			end
		end
	...
	if self.pointer_x and self.drawpointer then
		local w=lstg.world
		SetViewMode'ui'
		local x,y=WorldToScreen(max(min(self.pointer_x,w.r-24),w.l+24),w.b)
		Render('boss_ui_pointer',x,w.scrb,0,0.5)
		SetViewMode'world'
	end
	--为boss ex时，下方逻辑不执行
	if self.boss.ex then
	...
	if self.drawspell then
		local w=lstg.world
		if self.sc_name~='' then
			SetImageState('boss_ui_sc_name_bg','',Color(alpha*255,255,255,255))
			Render('boss_ui_sc_name_bg',w.r+xoffset,w.t-dy2,0,0.5)
			RenderTTF('sc_name',self.sc_name,w.r+1+xoffset,w.r+1+xoffset,w.t+2-dy2,w.t+2-dy2,Color(alpha*255,0,0,0),'right','noclip')
			RenderTTF('sc_name',self.sc_name,w.r+xoffset,w.r+xoffset,w.t+3-dy2,w.t+3-dy2,Color(alpha*255,255,255,255),'right','noclip')
		end
		if self.boss.sc_bonus and self.sc_hist then
			local b
			if self.boss.sc_bonus>0 then b=string.format('%.0f',self.boss.sc_bonus-self.boss.sc_bonus%10) else b='FAILED ' end
			SetFontState('bonus','',Color(alpha*255,0,0,0))
			RenderText('bonus',b,w.r-6+xoffset,w.t-17-dy2,0.5,'right')
			RenderText('bonus',string.format('%02d/%02d',self.sc_hist[1],self.sc_hist[2]),w.r-89+xoffset,w.t-17-dy2,0.5,'right')
			RenderText('bonus','HISTORY        BONUS',w.r-55+xoffset,w.t-17-dy2,0.5,'right')
			SetFontState('bonus','',Color(alpha*255,255,255,255))
			RenderText('bonus',b,w.r-6+xoffset,w.t-16-dy2,0.5,'right')
			RenderText('bonus',string.format('%02d/%02d',self.sc_hist[1],self.sc_hist[2]),w.r-88+xoffset,w.t-16-dy2,0.5,'right')
			RenderText('bonus','HISTORY        BONUS',w.r-56+xoffset,w.t-16-dy2,0.5,'right')
		end
	end
	--非符、符卡时间
	if self.is_combat and self.drawtime then
		...
		local yoffset
		if self.boss.sc_bonus then yoffset=max(20-self.boss.timer,0) else yoffset=min(20,(self.boss.timer+1)) end
		if self.countdown>=9.0 then
			SetFontState('time','',Color(alpha1*255,255,255,255))
			RenderText('time',string.format('%02d',int(self.countdown)),4,192+yoffset+_dy-dy2,0.5,'vcenter','right')
			RenderText('time','.'..string.format('%d%d',min(9,cd/10),min(9,cd%10)),4,189+yoffset+_dy-dy2,0.3,'vcenter','left')
		else
			SetFontState('time','',Color(alpha1*255,255,30,30))
			RenderText('time',string.format('%02d',min(99.99,int(self.countdown))),4,192+yoffset+_dy-dy2,0.5,'vcenter','right')
			RenderText('time','.'..string.format('%d%d',min(9,cd/10),min(9,cd%10)),4,189+yoffset+_dy-dy2,0.3,'vcenter','left')
		end
	end
end
...
LoadTexture('boss_ui_th',patch..'boss_ui_th.png')
LoadImage('boss_ui_sc_name_bg','boss_ui_th',0,0,512,96)
SetImageCenter('boss_ui_sc_name_bg',512,32)
LoadImage('boss_ui_pointer','boss_ui_th',0,120,96,32)
SetImageCenter('boss_ui_pointer',48,0)
LoadImage('boss_ui_sc_left','boss_ui_th',64,152,64,64)
