...
function boss.dialog:del()
    local players=Players(self)--自机活多玩家适配
    for i=1,#players do
...
function dialog_displayer:frame()
    ...
    if player.dialog==true and self.active then
        if KeyIsDown('shoot',1) or KeyIsDown('shoot',2) then self.jump_dialog=self.jump_dialog+1 else self.jump_dialog=0 end
    end
end
function dialog_displayer:oldrender()
    if self.active then
        SetViewMode'ui'
...
function dialog_displayer:render()--已适配宽屏--自机活版本
    if self.active then
        SetViewMode'ui'
            local w=lstg.world
